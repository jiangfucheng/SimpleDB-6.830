...
public class QueryPlanVisualizer {
    ...
    private int calculateQueryPlanTreeDepth(simpledb.OpIterator root) {
        if (root == null)
            return 0;

        if (!(root instanceof simpledb.Operator))
            return 2;
        simpledb.Operator o = (simpledb.Operator) root;
        simpledb.OpIterator[] children = o.getChildren();

        if (o instanceof simpledb.Join || o instanceof simpledb.HashEquiJoin) {
    ...
    private SubTreeDescriptor buildTree(int queryPlanDepth, int currentDepth,
                                        simpledb.OpIterator queryPlan, int currentStartPosition,
        ...
        if (queryPlan instanceof simpledb.SeqScan) {
            simpledb.SeqScan s = (simpledb.SeqScan) queryPlan;
...
//            TupleDesc td = s.getTupleDesc();
            ...
            simpledb.Operator plan = (simpledb.Operator) queryPlan;
            simpledb.OpIterator[] children = plan.getChildren();

            if (plan instanceof simpledb.Join) {
                simpledb.Join j = (simpledb.Join) plan;
                simpledb.TupleDesc td = j.getTupleDesc();
                simpledb.JoinPredicate jp = j.getJoinPredicate();
            ...
            } else if (plan instanceof simpledb.HashEquiJoin) {
                simpledb.HashEquiJoin j = (simpledb.HashEquiJoin) plan;
                simpledb.JoinPredicate jp = j.getJoinPredicate();
                simpledb.TupleDesc td = j.getTupleDesc();
                String field1 = td.getFieldName(jp.getField1());
                String field2 = td.getFieldName(jp.getField2()
                ...
                thisNode.rightChild = right;
                thisNode.height = currentDepth;
            } else if (plan instanceof simpledb.Aggregate) {
                simpledb.Aggregate a = (simpledb.Aggregate) plan;
                int upBarShift = parentUpperBarStartShift;
                String alignTxt;
                simpledb.TupleDesc td = a.getTupleDesc();
                int gfield = a.groupField();

                if (gfield == simpledb.Aggregator.NO_GROUPING) {
                ...
                thisNode.leftChild = child;
                thisNode.height = currentDepth;
            } else if (plan instanceof simpledb.OrderBy) {
                simpledb.OrderBy o = (simpledb.OrderBy) plan;
                ...
                thisNode.leftChild = child;
                thisNode.height = currentDepth;
            } else if (plan instanceof simpledb.Project) {
                simpledb.Project p = (simpledb.Project) plan;
                String fields = "";
    ...
    public String getQueryPlanTree(simpledb.OpIterator physicalPlan) {
    ...
    public void printQueryPlanTree(simpledb.OpIterator physicalPlan, PrintStream out) {
        if (out == null)
            out = System.out;
